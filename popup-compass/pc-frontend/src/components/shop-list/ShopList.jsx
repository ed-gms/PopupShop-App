...
class ShopList extends Component {
  ...
  render() {
    let filteredShops = this.props.shops.filter((shop) => {
      return shop.name.toLowerCase().indexOf(this.state.search.toLowerCase()) !== -1;
    });
        ...
        <div key={i} className="col-md-4 mb-4">
          <div className="view overlay z-depth-1-half" >
            <img
              ...
              className="shops-imgs img-fluid" alt=""
            />
          ...
          <Link to={`/shopdetails/${shopID}`}>
            <h5 className="title" style={titleText}>{shop.name}</h5>
          </Link>
          <p>
    ...
    return (
      <div style={pageStyle}>
        <div className="container">
          <SearchBar shops={this.props.shops} search={this.state.search} updateSearch={this.updateSearch} />
          <div className="row">
            {filteredJSX}
          </div>
        </div>
